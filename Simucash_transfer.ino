...
void loop(void) {
  ...
  int tAmount;                              // Transfer amount to the card
  ...
  int8_t data[16];                          // Array to store block data during reads
  int currentblock = 4;                     // Block with card stored value

  // Use the default NDEF keys (these would have have set by mifareclassic_formatndef.pde!)
  uint8_t keya[6] = { 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5 };
  uint8_t keyb[6] = { 0xD3, 0xF7, 0xD3, 0xF7, 0xD3, 0xF7 };

  Serial.println("Enter the amount to send to your card:");
  // Wait for use to input the amount to update with
  while (!Serial.available());
  // Amount was input
  tAmount = Serial.parseInt();
  Serial.print("You have requested to transfer: ")+Serial.println(tAmount,DEC);
  if(tAmount<16){
    num[14] = 0;
    num[15] = tAmount;
  }else{
    num[15] = (byte)(tAmount & 0xff);
    num[14] = (byte)(tAmount >> 8);
  }
  // Serial.print("num: ")+Serial.println(num[14],HEX)+Serial.println(",")+Serial.println(num[15],HEX);
  Serial.flush();

  // Get Data for Block 5
  
  
  Serial.println("Please place your Simucash card on the NFC terminal");
...
//      Serial.print("tAmount: ")+Serial.println(tAmount,DEC);
      ...
      // Check that there is enough spece on the stored value card to make the transfer
      if ((storedAmounti+tAmount) <= 10000){
        // do transfer
        ...
        Serial.print("Your card now has:"); Serial.println(storedAmountf,DEC); // Serial.println(storedAmounti,DEC); Serial.println(tAmount,DEC); Serial.println(storedAmounti+tAmount,DEC);
      }else{
        Serial.println("Transfer incomplete. Your card will be above it's maximum limit.");
      }
    }else{
...
//    {
//      // Read successful
//      Serial.print("Block ");Serial.print(4, DEC);
//      Serial.print("  ");
//      // Dump the raw data
//      nfc.PrintHex(data, 16);
//      // Serial.println(String(data[0],HEX)+String(data[1],HEX));
//    }
